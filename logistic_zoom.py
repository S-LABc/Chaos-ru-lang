...
# Размеры окна (для рендера из видео 2538 на 1080)
window =  {
    'width':"1280",
    'hight':"720",
}
...
gens = 100 # для рендера из видео 1000
rates = 5000 # для рендера из видео 20000

rec_prefix = './frames'
project_name = 'logistic_zoom'

# Добавление строки project_name включит отображение бифуркационных меток (для получения постоянной Фейгенбаума)
# В оригинале эта строка включла только метки, отключая график. Если нужно как в оригинале, то смотрите строку 233
...
def feigen_ruler(plt, parent, color, x0, y0, x1, y1, x2=None, y2=None):
    ...
    if x2 != None and y2 != None:
        # Расчет и отображение постоянной Фейгенбаума
        f = (x1-x2)/(x0-x1)
        label = f"{round(f,5)}"
        
        # так было в оригинале (отображение выключено)
        #f = x0-x1 
        #label = f""
...
def zoom_plot(target, RATES, ENDS, first=False):
    ...
    if not first and not 'labels' in project_name:
        start = time()
        print('... Симуляция в диапазоне', RATES, ENDS, 'gens =', gens, 'rates =', rates)
        pops = simulate(num_gens=gens, num_rates=rates,

                                rate_min=RATES[0], rate_max=RATES[1],

                                num_discard = 1000, initial_pop=0.5)
        print('>>> ГОТОВО', round(time()-start,2),'с')
        
    # Если нужно только отображение бифуркационных меток (так было в оригинале, но мне показаось неинтересным)
    #elif first and 'labels' in project_name:
    elif not first and 'labels' in project_name:
...
class Figure(vp.Fig):
    ...
    def on_timer(self, event):
        ...
        if not hasattr(self, 'plotted'):
            ...
            if self.rec:
                ...
                print(f'>>> КАДР: {project_name}_{self.f}.png, ЗАТРАЧЕНО ВРЕМЕНИ',
    ...
    def done(self):
        ...
        if self.rec:
            ...
            print("Логистическое отображение с приближением выполнено")
            exit()
...
fig = Figure(show=False, title="Логистическое отображение с приближением", size=(window['width'], window['hight']),
                record=rec_dict)
