...
def series_plot(plt, y_vals, idx=100, r=0, xall=False):
    ...
    plt.clear()
    plt.setTitle("Популяция от времени, r: {:4.2f}".format(r))
...
class Controls(QWidget):
    def __init__(self, variable='', parent=None):
        ...
        self.cobweb_box = QtWidgets.QCheckBox('🕸️ График\nпопуляций', parent=self)
        self.l1.addWidget(self.cobweb_box)
        self.cobweb_box.setChecked(0)

        self.series_box = QtWidgets.QCheckBox("📈 График\nфункции", parent=self)
        self.l1.addWidget(self.series_box)
        self.series_box.setChecked(1)

        self.bifurc_box = QtWidgets.QCheckBox("❄️ График\nбифуркаций", parent=self)
        ...
        self.animlabel.setText("Скорость анимации")
        ...
        self.rate_txt = QLabel("Коэффициент\nроста, r", self)
        ...
        self.ipoptxt.setText("Начальная\nпопуляция, x")
        ...
        self.setFixedWidth(self.animrate.width() + 50)
        ...
        self.animb = QtWidgets.QPushButton('Проиграть', parent=self)
        self.l5.addWidget(self.animb)
        self.animb.setFixedWidth(self.animlabel.width())

        self.reset = QtWidgets.QPushButton('Сбросить', parent=self)
        self.l5.addWidget(self.reset)
        self.reset.setFixedWidth(self.animlabel.width())

        self.clear = QtWidgets.QPushButton('Очистить', parent=self)
...
class Widget(QWidget):
    def __init__(self, app, parent=None):
        ...
        self.setWindowTitle("Логистическое отображение 🤯")
        self.horizontalLayout.addWidget(self.win)

        self.plots = [
                        self.win.addPlot(col=1, title="Cobwebb",
                                        labels={'left':"Популяция следующая",
                                                'bottom':"Популяция нынешняя"}),

                        self.win.addPlot(col=2, title="Популяция от времени",
                                        labels={'left':"Популяция",
                                                'bottom':"Время"}),

                        self.win.addPlot(col=3, title="Бифуркации",
                                        labels={'left':"Стационарное значение",
                                                'bottom':"Коэф. роста"}),
        ]
    ...
    def animate_toggle(self):
        ...
        if self.animate:
            print('Проигрывание')
            self.update_plot()
            self.controls.animb.setText('Пауза')
        else:
            print("Пауза")
            self.timer.stop()
            self.controls.animb.setText('Проиграть')
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == 32: # Space
            ...
            print(f'Недопустимая клавиша: {event.key()}, "{event.text()}"')
