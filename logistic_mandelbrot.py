"""
    ...
    Изменил Скляр Роман S-LAB, 2022
        https://github.com/S-LABc/
...
def ffmpeg():
    ...
    print('КОНВЕРТИРОВАНИЕ >>>', convert_cmd)
    ...
    print(f"{project_name} просчитывается")
...
f = 0 # Номер начального кадра

F = 24*30 // 2 # Количество кадров за одно перемещение камеры

D = 100 # Размеры вокселей (для рендера из видео 1000)

Omax =  1 # Количество итераций передискретизации (для рендера из видео 16)

#Omax seems to scale as: time to render in seconds = (3*o)**2.7 @ D=1000
# on a 64 core machine at 2.7GHz per core

I = 1500 # Максимаьное количество итераций Мандельброта

rec_size = (1280, 720) # Размеры окна (для рендера из видео 2538 на 1080)

# Куда сохранять кадры
rec_prefix = './frames'
project_name = f'fractal_mandelbrot_{D}'
frame_dir = Path(f'{rec_prefix}/{project_name}')

# Соранять кадры в .png?
rec = False

# Воспроизведение кадров?
play = True

# Генерировать данные? (True если меняете D или Omax или I)
generate = False
...
if not generate:
    ...
    print("Создание маски для множества Мандельброта", time()-start)
    ...
    print("НАЧИНАЮ... D=",D," O=",O)
    if D == 1000:
        print('ПОТРЕБУЕТСЯ ВРЕМЕНИ...', (100*(3*O/10)**2.7)/(60*60),'ч' )

    print(f"{project_name} с параметрами D={D}, I={I}, O={O} начал генерироваться")

    vol = make_triplebrot(mset, D, D, D, I, oversample=O)
    print('ВЫПОЛНЕНО ЗА', time()-start, "с")

    GB = vol.size * vol.itemsize * 1e-9
    print("РАЗМЕР ФАЙЛА =", GB, "ГБ")
    np.savez(datafile, data=vol)
    print("ОЧИШЕНО!")

    print(f"{project_name} с параметрами D={D}, I={I}, O={O} создан и очищен"
                f" {GB}ГБ за {time()-start}с")
...
# Prepare canvas
canvas = scene.SceneCanvas(title='Множество Мандельброта с логистическим отображением', keys='interactive', size=rec_size, show=not rec)
...
# Начальное расстояние от камеры до объекта (определено эмпирически)
Ds = 8000 * vol.shape[0]/100 # изначальноо было 5000
...
def camera_move(camera, f, F):
    ...
    for k in keyframes:
        try:
            ...
            print('Несоответствие длины для', k)
...
def update(event):
    ...
    if rec:
        ...
        print('Сохранен кадр', f, 'затрачено времени:', ETA)
